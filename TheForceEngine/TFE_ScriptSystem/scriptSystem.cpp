...
namespace TFE_ScriptSystem
{
	...
	void TFE_Print(std::string& msg)
	{
		TFE_System::logWrite(LOG_MSG, "Scripting", "%s", msg.c_str());
	}
	
	void TFE_PrintInt(std::string& msg, s32 value0)
	{
		TFE_System::logWrite(LOG_MSG, "Scripting", "%s %d", msg.c_str(), value0);
	}

	void TFE_PrintUint(std::string& msg, u32 value0)
	{
		TFE_System::logWrite(LOG_MSG, "Scripting", "%s %u", msg.c_str(), value0);
	}

	void TFE_PrintFloat(std::string& msg, f32 value0)
	{
		TFE_System::logWrite(LOG_MSG, "Scripting", "%s %f", msg.c_str(), value0);
	}
	...
	bool init()
	{
		...
		if (!s_engine)
		{
			TFE_System::logWrite(LOG_ERROR, "Scripting", "Failed to create scripting engine.");
			return false;
		}
		...
		res = s_engine->RegisterGlobalFunction("void TFE_Print(const string &in)", asFUNCTION(TFE_Print), asCALL_CDECL); assert(res >= 0);
		res = s_engine->RegisterGlobalFunction("void TFE_Print(const string &in, int value)", asFUNCTION(TFE_PrintInt), asCALL_CDECL); assert(res >= 0);
		res = s_engine->RegisterGlobalFunction("void TFE_Print(const string &in, uint value)", asFUNCTION(TFE_PrintUint), asCALL_CDECL); assert(res >= 0);
		res = s_engine->RegisterGlobalFunction("void TFE_Print(const string &in, float value)", asFUNCTION(TFE_PrintFloat), asCALL_CDECL); assert(res >= 0);
	...
	bool loadScriptModule(ScriptType type, const char* moduleName)
	{
		...
		if (res < 0)
		{
			TFE_System::logWrite(LOG_ERROR, "Scripting", "Unrecoverable error while starting a new module.");
			return false;
		}
		...
		if (res < 0)
		{
			...
			TFE_System::logWrite(LOG_ERROR, "Scripting", "Cannot open or parse file.");
			return false;
		}
		...
		if (res < 0)
		{
			...
			TFE_System::logWrite(LOG_ERROR, "Scripting", "Compile error(s).");
			return false;
		}
		return true;
	}
	...
	void executeScriptFunction(ScriptType type, void* funcPtr)
	{
		...
		if (res != asEXECUTION_FINISHED)
		{
			...
			if (res == asEXECUTION_EXCEPTION)
			{
				...
				TFE_System::logWrite(LOG_ERROR, "Scripting", "An exception '%s' occurred. Please correct the code and try again.\n", s_context[type]->GetExceptionString());
			}
		}
	}
	...
	void messageCallback(const asSMessageInfo *msg, void *param)
	{
		...
		TFE_System::logWrite(type, "Scripting", "%s (%d, %d) : %s\n", msg->section, msg->row, msg->col, msg->message);
	}
}
