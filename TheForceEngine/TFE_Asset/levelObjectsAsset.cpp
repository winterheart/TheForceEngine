...
namespace TFE_LevelObjects
{
	...
	bool load(const char* name)
	{
		...
		if (!parseObjects())
		{
			TFE_System::logWrite(LOG_ERROR, "Level", "Failed to parse O file \"%s\" from GOB \"%s\"", name, gobPath);
	...
	void save(const char* name, const char* path)
	{
		...
		outFile.writeString("/* ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~\r\n");
		outFile.writeString("This file has been created by the Force Engine from %s.\r\n\r\n", name);
		outFile.writeString("~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~*/\r\n\r\n");
		...
		for (u32 i = 0; i < objCount; i++, obj++)
		{
			...
			for (u32 l = 0; l < logicCount; l++, logic++)
			{
				...
				{
					outFile.writeString("            VUE: %s \"%s\"\r\n", logic->vue->name, TFE_VueAsset::getTransformName(logic->vue, logic->vueId));
					if (logic->vueAppend)
					{
	...
	bool parseObjects()
	{
		...
		while (bufferPos < len)
		{
			...
			else if (sequence)
			{
				...
				{
					logic->vue = TFE_VueAsset::get(tokens[1].c_str());
					logic->vueId = -1;
					if (logic->vue)
					{
						logic->vueId = tokens.size() > 2 ? TFE_VueAsset::getTransformIndex(logic->vue, tokens[2].c_str()) : 0;
					}
				}
				else if (logic && strcasecmp("VUE_APPEND:", tokens[0].c_str()) == 0)
				{
					logic->vueAppend = TFE_VueAsset::get(tokens[1].c_str());
					logic->vueAppendId = -1;
					if (logic->vueAppend)
					{
						logic->vueAppendId = tokens.size() > 2 ? TFE_VueAsset::getTransformIndex(logic->vueAppend, tokens[2].c_str()) : 0;
					}
				}
			}
		}
