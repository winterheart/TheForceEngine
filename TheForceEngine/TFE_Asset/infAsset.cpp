...
namespace TFE_InfAsset
{
	...
	bool load(const char* name)
	{
		...
		if (!parseInf())
		{
			TFE_System::logWrite(LOG_ERROR, "INF", "Failed to parse INF file \"%s\" from GOB \"%s\"", name, gobPath);
	...
	void finishClass(InfItem* item, InfClassData* curClass, const u32* clients, const u16* slaves, const InfStop* stops, const InfFuncTemp* funcs, u32 clientCount, u32 slaveCount, u32 stopCount, u32 funcCount, s32 mergeStart, bool overrideAsExp)
	{
		...
		else if (curClass->iclass == INF_CLASS_ELEVATOR && (curClass->isubclass == ELEVATOR_DOOR || curClass->isubclass == ELEVATOR_DOOR_INV || curClass->isubclass == ELEVATOR_DOOR_MID) && stopCount == 0)
		{
			...
			if (sectorId < 0xffff)
			{
				Sector* sector = TFE_LevelAsset::getLevelData()->sectors.data() + sectorId;
		...
		else
		{
			...
			for (u32 f = 0; f < funcCount; f++)
			{
				...
				{
					TFE_System::logWrite(LOG_ERROR, "INF", "Function references an invalid stop %u.", s);
				}
			}
		}
	}
	...
	bool parseInf()
	{
		...
		while (bufferPos < len)
		{
			...
			else if (strcasecmp("item:", tokens[0].c_str()) == 0)
			{
				...
				{
					if (s_data.itemCount)
						TFE_System::logWrite(LOG_ERROR, "INF", "Too many items - only %u were specified in the ITEMS field.", s_data.itemCount);
					else
						TFE_System::logWrite(LOG_ERROR, "INF", "The ITEMS field is missing from the INF file.");
					break;
				}
				...
				for (size_t t = 2; t < tokens.size();)
				{
					if (strcasecmp(tokens[t].c_str(), "name:") == 0)
					{
						...
						if (sectorId < 0)
						{
							TFE_System::logWrite(LOG_WARNING, "INF", "Inf Item \"%s\" does not match a sector in the level.", name);
						}
					...
					else if (strcasecmp(tokens[t].c_str(), "num:") == 0)
					{
						...
						{
							TFE_System::logWrite(LOG_WARNING, "INF", "Inf Item \"%s\" is a line type but does not have a valid line: %d", name ? name : "null", wallNum);
						}
						if (wallNum >= 0)
						{
			...
			else if (strcasecmp("seq", tokens[0].c_str()) == 0)
			{
				if (!curItem)
				{
					TFE_System::logWrite(LOG_ERROR, "INF", "Beginning the sequence with no item.");
					continue;
				}
			...
			else if (strcasecmp("seqend", tokens[0].c_str()) == 0)
			{
				...
				if (!curItem)
				{
					TFE_System::logWrite(LOG_ERROR, "INF", "Ending the sequence with no item.");
					continue;
				}
			...
			else if (strcasecmp("stop:", tokens[0].c_str()) == 0 && curClass != nullptr)
			{
				...
				{
					TFE_System::logWrite(LOG_ERROR, "INF", "Too many stops %u of %u.", stopCount + 1, MAX_STOPS);
					continue;
				}
				//[1] = value0
				//[2] = value1
				...
				else
				{
					...
					if (sectorId < 0)
					{
						TFE_System::logWrite(LOG_ERROR, "INF", "Stop #%u has the sectorname type but \"%s\" has not been found in the level.", stopNum, value0);
					}
			...
			else if (strcasecmp("client:", tokens[0].c_str()) == 0 && tokens.size() >= 2 && curClass != nullptr)
			{
				...
				if (sectorId < 0)
				{
					TFE_System::logWrite(LOG_ERROR, "INF", "Trigger client specified \"%s\" but cannot be found in the level.", recStr);
				}

				assert(clientIndex < MAX_CLIENTS);
				if (clientIndex >= MAX_CLIENTS)
				{
					TFE_System::logWrite(LOG_ERROR, "INF", "Too many trigger clients %u, max is %u.", clientIndex+1, MAX_CLIENTS);
				}
				else
				{
			...
			else if (strcasecmp("message:", tokens[0].c_str()) == 0 && curClass != nullptr)
			{
				...
				{
					TFE_System::logWrite(LOG_ERROR, "INF", "Too many functions %u of %u.", funcCount + 1, MAX_FUNC);
					continue;
				}
				...
				else if (curClass->iclass == INF_CLASS_ELEVATOR)
				{
					...
					else
					{
						...
						if (sectorId < 0)
						{
							TFE_System::logWrite(LOG_ERROR, "INF", "Message client sector \"%s\" has not been found in the level.", recStr);
							sectorId = 0xffff;
						}
				...
				{
					TFE_System::logWrite(LOG_ERROR, "INF", "Messages can only be sent from Elevators or Triggers.");
				}
			}
			...
			else if (strcasecmp("page:", tokens[0].c_str()) == 0)
			{
				...
				curFunc->func.arg[0].iValue = TFE_VocAsset::getIndex(tokens[2].c_str());
			}
			...
			else if (strcasecmp("master:", tokens[0].c_str()) == 0)
			{
				...
				if (!curClass) { TFE_System::logWrite(LOG_ERROR, "INF", "Assigning the variable \"%s\" but there is no class.", tokens[0].c_str()); continue; }
			...
			else if (strcasecmp("sound:", tokens[0].c_str()) == 0)
			{
				...
				if (tokens.size() == 3)
				{
					...
					if (index >= 0)
					{
						...
						{
							curClass->var.sound[index] = TFE_VocAsset::getIndex(tokens[2].c_str());
						}
					}
				}
				else
				{
					curClass->var.sound[0] = TFE_VocAsset::getIndex(tokens[1].c_str());
				}
			}
			...
			else if (strcasecmp("slave:", tokens[0].c_str()) == 0)
			{
				...
				if (sectorId >= 0)
				{
					...
					{
						TFE_System::logWrite(LOG_ERROR, "INF", "Too many trigger slaves %u, max is %u.", slaveCount + 1, MAX_SLAVES);
					}
					else
					{
			...
			else
			{
				...
				TFE_System::logWrite(LOG_WARNING, "INF", "Unrecognized command \"%s\"", tokens[0].c_str());
			}
		}
		...
		TFE_System::logWrite(LOG_MSG, "INF", "Inf memory used: %u bytes.", s_memoryPool.getMemoryUsed());
		return true;
	}
	...
	void setupClassDefaults(InfClassData* classInfo)
	{
		...
		if (classInfo->iclass == INF_CLASS_ELEVATOR)
		{
			...
			{
				classInfo->var.sound[0] = TFE_VocAsset::getIndex("ELEV2-1.VOC");
				classInfo->var.sound[1] = TFE_VocAsset::getIndex("ELEV2-2.VOC");
				classInfo->var.sound[2] = TFE_VocAsset::getIndex("ELEV2-3.VOC");
			}
			...
			{
				classInfo->var.sound[0] = TFE_VocAsset::getIndex("DOOR2-1.VOC");
				classInfo->var.sound[1] = TFE_VocAsset::getIndex("DOOR2-2.VOC");
				classInfo->var.sound[2] = TFE_VocAsset::getIndex("DOOR2-3.VOC");
			}
			else if (subclass == ELEVATOR_DOOR)
			{
				classInfo->var.sound[0] = TFE_VocAsset::getIndex("DOOR.VOC");
			}
		}
		else if (classInfo->iclass == INF_CLASS_TRIGGER)
		{
			...
			{
				classInfo->var.sound[0] = TFE_VocAsset::getIndex("SWITCH3.VOC");
			}
		}
	}
	...
	InfMessage getInfMessage(const char* type)
	{
		...
		TFE_System::logWrite(LOG_ERROR, "INF", "Invalid message type: \"%s\"", type);
	...
	InfClass getInfClass(const char* str)
	{
		...
		TFE_System::logWrite(LOG_ERROR, "INF", "\"%s\" is not a valid item class.", str);
	...
	InfSubClass getInfSubClass(const char* str, InfClass iclass)
	{
		if (iclass == INF_CLASS_ELEVATOR)
		{
			...
			TFE_System::logWrite(LOG_ERROR, "INF", "\"%s\" is not a valid item subclass for \"Elevator.\"", str);
		...
		else if (iclass == INF_CLASS_TELEPORTER)
		{
			...
			TFE_System::logWrite(LOG_ERROR, "INF", "\"%s\" is not a valid item subclass for \"Teleporter.\"", str);
			return SUBCLASS_NONE;
		}

		TFE_System::logWrite(LOG_ERROR, "INF", "Invalid item class ID %u", iclass);
		return SUBCLASS_NONE;
	}
};
