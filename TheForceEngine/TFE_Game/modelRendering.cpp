...
namespace TFE_ModelRender
{
	...
	// API
	////////////////////////////////
	void init(TFE_Renderer* renderer, s32 width, s32 height, f32 focalLength, f32 heightScale)
	{
		s_renderer = renderer;
		s_width = width;
		s_height = height;
	...
	bool computeScreenSize(const Model* model, const Vec3f* modelOrientation, const Vec3f* modelPos, const Mat3* optionalTransform, const Vec3f* cameraPos, const f32* cameraRot, f32 heightOffset, Vec2i* screenRect)
	{
		...
		{
			TFE_Math::buildRotationMatrix({ roll, yaw, pitch }, mat33);
		}
	...
	void draw(const Model* model, const Vec3f* modelOrientation, const Vec3f* modelPos, const Mat3* optionalTransform, const Vec3f* cameraPos, const f32* cameraRot, f32 heightOffset, const Sector* sector)
	{
		...
		for (u32 i = 0; i < model->polygonCount; i++, sorted++)
		{
			...
			if (sorted->polyIndex < triCount)
			{
				...
				const s32 lightLevel = TFE_RenderCommon::lightFalloff(ambient, sorted->sortDepth);
				...
				if (hasTexturing && tri->shading == SHADING_GOURTEX)
				{
					...
					const s32 l0 = TFE_RenderCommon::lightFalloff(ambient, v0->z);
					const s32 l1 = TFE_RenderCommon::lightFalloff(ambient, v1->z);
					const s32 l2 = TFE_RenderCommon::lightFalloff(ambient, v2->z);
			...
			else
			{
				...
				if (hasTexturing && quad->shading == SHADING_GOURTEX)
				{
					...
					const s32 l1 = TFE_RenderCommon::lightFalloff(ambient, v1->z);
					const s32 l2 = TFE_RenderCommon::lightFalloff(ambient, v2->z);
					const s32 l3 = TFE_RenderCommon::lightFalloff(ambient, v3->z);
	...
	void drawPolygon(ClipPolygon* clipPoly, u8 color, const TextureFrame* texture, s32 ambient)
	{
		...
		if (planeMapped)
		{
			...
			for (s32 y = start; y < yCount; y++)
			{
				...
				const s32 lightLevel = TFE_RenderCommon::lightFalloff(ambient, z);
