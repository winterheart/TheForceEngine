...
namespace LevelEditorData
{
	...
	EditorTexture* createTexture(const Texture* src)
	{
		...
		tex->texture = TFE_RenderBackend::createTexture(src->frames[0].width, src->frames[0].height, trueColor);
	...
	EditorTexture* createObjectTexture(ObjectClass oclass, const char* dataFile)
	{
		...
		switch (oclass)
		{
			...
			case CLASS_SPRITE:
			{
				const Sprite* sprite = TFE_Sprite::getSprite(dataFile);
				if (sprite)
				{
					s_objIcons.push_back({});
					tex = &s_objIcons.back();
					...
					tex->texture = TFE_RenderBackend::createTexture(sprite->frames[0].width, sprite->frames[0].height, trueColor);
			...
			case CLASS_FRAME:
			{
				const Frame* frame = TFE_Sprite::getFrame(dataFile);
				if (frame)
				{
					s_objIcons.push_back({});
					tex = &s_objIcons.back();
					...
					tex->texture = TFE_RenderBackend::createTexture(frame->width, frame->height, trueColor);
	...
	void freeTextures()
	{
		...
		{
			TFE_RenderBackend::freeTexture(s_editorLevel.textures[t].texture);
		}
		const size_t iconCount = s_objIcons.size();
		for (size_t t = 0; t < iconCount; t++)
		{
			TFE_RenderBackend::freeTexture(s_objIcons[t].texture);
		}
		s_objIcons.clear();
	...
	void loadTextures(const Palette256* pal)
	{
		...
		char textureGobPath[TFE_MAX_PATH];
		TFE_Paths::appendPath(TFE_PathType::PATH_SOURCE_DATA, "TEXTURES.GOB", textureGobPath);
		...
		for (u32 i = TEXTURES_GOB_START_TEX; i < fileCount; i++)
		{
			...
			const Texture* texture = TFE_Texture::get(name);
			...
			TextureGpu* textureGpu = TFE_RenderBackend::createTexture(w, h, trueColor);
	...
	s32 loadRuntimeTexture(const char* name, LevelData* output)
	{
		...
		s32 texId = -1;
		Texture* tex = TFE_Texture::get(name);
		if (tex)
		{
			texId = (s32)texCount;
	...
	bool generateLevelData()
	{
		...
		TFE_LevelAsset::unload();
		LevelData* output = TFE_LevelAsset::getLevelData();
	...
	{
		TFE_LevelObjects::unload();
		LevelObjectData* output = TFE_LevelObjects::getLevelObjectData();
	...
	bool generateInfAsset()
	{
		...
		TFE_InfAsset::unload();
		InfData* output = TFE_InfAsset::getInfData();
		MemoryPool* memoryPool = TFE_InfAsset::getMemoryPool(true);

		output->itemCount = (u32)s_editorInf.item.size();
		output->completeId = -1;

		const u32 secFlagDoorCount = TFE_InfAsset::countSectorFlagDoors();
	...
	void triangulateSector(const EditorSector* sector, SectorTriangles* outTri)
	{
		...
		u32 triCount = 0;
		Triangle* triangle = TFE_Polygon::decomposeComplexPolygon(contourCount, contours, &triCount);
	...
	bool traceRay(const Ray* ray, RayHitInfoLE* hitInfo)
	{
		...
		if (ray->objSelect)
		{
			...
			f32 hitDist = (hitInfo->hitSectorId >= 0) ? TFE_Math::distance(&ray->origin, &hitInfo->hitPoint) : ray->maxDist;
	...
	void convertObjectsToEditor(const LevelObjectData* objData)
	{
		...
		for (u32 i = 0; i < objData->objectCount; i++, srcObj++)
		{
			...
			if (sectorId >= 0)
			{
				...
				{
					dstObj.displayModel = TFE_Model::get(dstObj.dataFile.c_str());
					...
					TFE_Math::buildRotationMatrix({ roll, yaw, pitch }, dstObj.rotMtx.m);
					dstObj.rotMtxT = TFE_Math::transpose(dstObj.rotMtx);
				}
				else
				{
