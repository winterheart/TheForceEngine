...
namespace TFE_JediRenderer
{
	...
	void TFE_Sectors_GPU::setupWallDrawFlags(RSector* sector)
	{
	...
	void TFE_Sectors_GPU::adjustHeights(RSector* sector, decimal floorOffset, decimal ceilOffset, decimal secondHeightOffset)
	{
		// Adjust objects.
	...
	void TFE_Sectors_GPU::computeBounds(RSector* sector)
	{
		RWall* wall = sector->walls;
		vec2* w0 = wall->w0;
		f32 maxX = w0->x.f32;
	...
	void TFE_Sectors_GPU::copy(RSector* out, const Sector* sector, const SectorWall* walls, const Vec2f* vertices, Texture** textures)
	{
	...
	void TFE_Sectors_GPU::update(u32 sectorId, u32 updateFlags)
	{
	...
	void TFE_Sectors_GPU::adjoin_setupAdjoinWindow(s32* winBot, s32* winBotNext, s32* winTop, s32* winTopNext, EdgePair* adjoinEdges, s32 adjoinCount)
	{
	#if 0
	...
	void TFE_Sectors_GPU::adjoin_computeWindowBounds(EdgePair* adjoinEdges)
	{
	#if	0
